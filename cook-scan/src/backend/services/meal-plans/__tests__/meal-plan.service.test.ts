...
function createMealPlan(overrides: Partial<MealPlanWithItems> = {}): MealPlanWithItems {
...
function createUpsertedMealPlan(overrides: Partial<UpsertedMealPlan> = {}): UpsertedMealPlan {
  const planId = overrides.id ?? "plan-1";
...
function createMealPlanRecord(overrides: Partial<MealPlanRecord> = {}): MealPlanRecord {
  return {
...
describe("meal-plan.service", () => {
  ...
  describe("addMealPlanItem", () => {
    it("正常系: 献立プランにアイテムを追加できる", async () => {
      ...
      vi.mocked(MealPlanRepository.createMealPlanItem).mockResolvedValueOnce(createMealPlanItem());
      ...
      expect(MealPlanRepository.upsertMealPlan).toHaveBeenCalledWith(
        "user-1",
        new Date(2026, 2, 2),
      );
  ...
  describe("removeMealPlanItem", () => {
    ...
    it("エラー: アイテムが見つからない場合はエラーを投げる", async () => {
      ...
      await expect(removeMealPlanItem("user-1", { itemId: "item-1" })).rejects.toThrow(
        "アイテムが見つかりません",
      );
  ...
  describe("generateShoppingList", () => {
    ...
    it("エラー: 献立プランが存在しない場合はエラーを投げる", async () => {
      ...
      await expect(generateShoppingList("user-1", { weekStart: "2026-03-02" })).rejects.toThrow(
        "献立プランにレシピが登録されていません",
      );
    });

...
describe("recipe.service", () => {
  ...
  describe("getRecipeById", () => {
    it("正常系: レシピIDとユーザーIDでレシピを取得できる", async () => {
      ...
      vi.mocked(RecipeRepository.findRecipeById).mockResolvedValueOnce(
        mockRecipe as RecipeDetailOutput,
      );
  ...
  describe("getRecipes", () => {
    it("正常系: ユーザーのレシピ一覧を取得できる", async () => {
      ...
      vi.mocked(RecipeRepository.findRecipesByUser).mockResolvedValueOnce(
        mockRecipes as RecipeListOutput[],
      );
  ...
  describe("createRecipe", () => {
    it("正常系: レシピを作成できる", async () => {
      ...
      expect(RecipeRepository.createIngredients).toHaveBeenCalledWith(
        mockTx,
        "recipe-new",
        input.ingredients,
      );
      expect(RecipeRepository.createSteps).toHaveBeenCalledWith(mockTx, "recipe-new", input.steps);
      expect(RecipeRepository.createRecipeTags).toHaveBeenCalledWith(mockTx, "recipe-new", [
        "tag-1",
      ]);
    });
  ...
  describe("updateRecipe", () => {
    it("正常系: レシピを更新できる", async () => {
      ...
      expect(RecipeRepository.checkRecipeOwnership).toHaveBeenCalledWith(
        "recipe-1",
        "user-1",
        mockTx,
      );
      expect(RecipeRepository.updateRecipe).toHaveBeenCalledWith(
        mockTx,
        "recipe-1",
        "更新レシピ",
        "更新メモ",

...
describe("tag.service", () => {
  ...
  describe("createTag", () => {
    ...
    it("カテゴリが見つからない場合はエラーを投げる", async () => {
      ...
      await expect(createTag(userId, { categoryId: "cat-unknown", name: "タグ" })).rejects.toThrow(
        "カテゴリが見つかりません",
      );
    ...
    it("他ユーザーのカテゴリにはタグを作成できない", async () => {
      ...
      await expect(createTag(otherUserId, { categoryId: "cat-1", name: "タグ" })).rejects.toThrow(
        "このカテゴリにタグを作成する権限がありません",
      );
  ...
  describe("updateTag", () => {
    ...
    it("タグが見つからない場合はエラーを投げる", async () => {
      ...
      await expect(updateTag(userId, { tagId: "tag-unknown", name: "名前" })).rejects.toThrow(
        "タグが見つかりません",
      );
    ...
    it("システムタグは編集できない", async () => {
      ...
      await expect(updateTag(userId, { tagId: "tag-system", name: "名前" })).rejects.toThrow(
        "システムタグは編集できません",
      );
    ...
    it("他ユーザーのタグは編集できない", async () => {
      ...
      await expect(updateTag(otherUserId, { tagId: "tag-1", name: "名前" })).rejects.toThrow(
        "このタグを編集する権限がありません",
      );
  ...
  describe("deleteTag", () => {
    ...
    it("システムタグは削除できない", async () => {
      ...
      await expect(deleteTag(userId, "tag-system")).rejects.toThrow("システムタグは削除できません");

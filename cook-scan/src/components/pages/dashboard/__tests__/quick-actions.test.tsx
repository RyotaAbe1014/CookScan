...
describe("QuickActions", () => {
  ...
  test("正常系：マイレシピカードが表示される", () => {
    ...
    expect(screen.getByText("保存したレシピを閲覧・編集")).toBeInTheDocument();
  });
  ...
  test("正常系：タグ管理カードが表示される", () => {
    ...
    expect(screen.getByText("カテゴリを作成してレシピを整理")).toBeInTheDocument();
  });
  ...
  test("正常系：買い物リストカードが表示される", () => {
    ...
    expect(screen.getByText("必要な食材をチェック管理")).toBeInTheDocument();
  });

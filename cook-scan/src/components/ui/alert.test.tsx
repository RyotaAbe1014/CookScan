...
describe("Alert", () => {
  ...
  test("正常系：各variantで対応するアイコンカラーが表示される", () => {
    ...
    const { container: errorContainer } = render(<Alert variant="error">エラー</Alert>);
    ...
    const { container: successContainer } = render(<Alert variant="success">成功</Alert>);
    ...
    const { container: warningContainer } = render(<Alert variant="warning">警告</Alert>);

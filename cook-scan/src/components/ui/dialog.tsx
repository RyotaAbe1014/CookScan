...
function DialogHeader({ children, className }: { children: React.ReactNode; className?: string }) {
  return <div className={cn("border-b border-gray-200 px-6 py-5", className)}>{children}</div>;
}
...
function DialogFooter({ children, className }: { children: React.ReactNode; className?: string }) {
  return (
    <div
      className={cn(
        "flex gap-3 border-t border-gray-200 bg-linear-to-r from-gray-50 to-white px-6 py-4",
        className,
      )}
    >
      {children}

...
describe("LoginForm", () => {
  ...
  test("正常系：フォームの全ての要素が正しくレンダリングされる", () => {
    ...
    expect(screen.getByText(/レシピをスキャンして/)).toBeInTheDocument();
  ...
  test("正常系：フッターメッセージが表示される", () => {
    ...
    expect(screen.getByText(/安全に保存されるあなたのレシピコレクション/)).toBeInTheDocument();
    expect(screen.getByText(/AIがレシピを自動抽出/)).toBeInTheDocument();
  });

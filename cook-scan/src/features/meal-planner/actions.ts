 ...
 * 週の献立プランを取得
 */
export async function getMealPlan(weekStart: string): Promise<Result<MealPlanOutput | null>> {
 ...
 * 献立プランから買い物リストを生成
 */
export async function generateShoppingList(weekStart: string): Promise<Result<{ count: number }>> {
  return withAuth(async (profile) => {
    try {

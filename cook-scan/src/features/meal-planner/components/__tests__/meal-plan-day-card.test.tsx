...
describe("MealPlanDayCard", () => {
  ...
  test("正常系：土曜日のカードが表示される", () => {
    render(
      <MealPlanDayCard {...defaultProps} dayOfWeek={5} date={new Date(2026, 2, 7)} items={[]} />,
    );
  ...
  test("正常系：日曜日のカードが表示される", () => {
    render(
      <MealPlanDayCard {...defaultProps} dayOfWeek={6} date={new Date(2026, 2, 8)} items={[]} />,
    );
  ...
  test("正常系：削除ボタンをクリックするとremoveMealPlanItemが呼ばれる", async () => {
    ...
    const deleteButtons = screen
      .getAllByRole("button")
      .filter((btn) => !btn.textContent?.includes("追加"));

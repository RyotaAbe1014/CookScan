...
describe("PasswordChangeForm", () => {
  ...
  it("全てのフォームフィールドをレンダリングする", () => {
    ...
    expect(
      screen.getByPlaceholderText(/8文字以上、大文字・小文字・数字を含む/i),
    ).toBeInTheDocument();
    expect(screen.getByPlaceholderText(/もう一度入力してください/i)).toBeInTheDocument();
  ...
  it("セキュリティ警告メッセージを表示する", () => {
    ...
    expect(
      screen.getByText(/パスワード変更後、すべてのデバイスから自動的にログアウトされます/i),
    ).toBeInTheDocument();
  });
  ...
  it("正しいデータでupdatePasswordを呼び出す", async () => {
    ...
    await user.type(screen.getByLabelText(/現在のパスワード/i), "OldPass123");
    await user.type(
      screen.getByPlaceholderText(/8文字以上、大文字・小文字・数字を含む/i),
      "NewPass123",
    );
    await user.type(screen.getByPlaceholderText(/もう一度入力してください/i), "NewPass123");
  ...
  it("失敗時にエラーメッセージを表示する", async () => {
    ...
    await user.type(screen.getByLabelText(/現在のパスワード/i), "WrongPass");
    await user.type(
      screen.getByPlaceholderText(/8文字以上、大文字・小文字・数字を含む/i),
      "NewPass123",
    );
    ...
    await waitFor(() => {
      expect(screen.getByText(/現在のパスワードが正しくありません/i)).toBeInTheDocument();
    });
  });
  ...
  it("処理中はローディングテキストを表示する", async () => {
    ...
    expect(screen.getByRole("button", { name: /パスワードを変更中/i })).toBeInTheDocument();
  });

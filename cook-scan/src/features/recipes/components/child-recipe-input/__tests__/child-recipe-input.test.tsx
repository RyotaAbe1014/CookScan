...
describe("ChildRecipeInput", () => {
  ...
  it("子レシピのタイトルが表示される", () => {
    render(<ChildRecipeInput item={mockItem} index={0} onUpdate={vi.fn()} onRemove={vi.fn()} />);
  ...
  it("分量を更新するとonUpdateが呼ばれる", async () => {
    ...
    render(<ChildRecipeInput item={mockItem} index={0} onUpdate={onUpdate} onRemove={vi.fn()} />);
  ...
  it("削除ボタンをクリックするとonRemoveが呼ばれる", async () => {
    ...
    render(<ChildRecipeInput item={mockItem} index={2} onUpdate={vi.fn()} onRemove={onRemove} />);

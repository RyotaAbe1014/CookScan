...
describe("useRecipeSearch", () => {
  ...
  it("isOpenがtrueになると検索が実行される", async () => {
    const { result } = renderHook(() => useRecipeSearch("parent-1", ["child-1"], true));

    await waitFor(() => {
      expect(result.current.isLoading).toBe(false);
    });

    expect(mockSearchAvailableRecipes).toHaveBeenCalledWith("parent-1", ["child-1"], undefined);
  ...
  it("handleSearchでクエリ付き検索ができる", async () => {
    const { result } = renderHook(() => useRecipeSearch("parent-1", [], true));
    ...
    expect(mockSearchAvailableRecipes).toHaveBeenLastCalledWith("parent-1", [], "レシピA");
  ...
  it("handleSearchKeyDownでEnterキーを押すと検索が実行される", async () => {
    ...
    expect(mockSearchAvailableRecipes).toHaveBeenCalledWith("parent-1", [], "テスト");
  });
  ...
  it("空文字のqueryはundefinedとして渡される", async () => {
    ...
    expect(mockSearchAvailableRecipes).toHaveBeenCalledWith("parent-1", [], undefined);
  });
});

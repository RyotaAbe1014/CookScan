...
export function useCookingTimer({
  ...
  const remainingSeconds = (() => {
    ...
    return calculateRemainingSeconds(totalSeconds, elapsedSeconds, runningSinceSeconds);
  })();

  // タイマー終了時の処理をメモ化
  const handleTimerComplete = useCallback(
    (completedStartedAt: number, totalSecondsValue: number) => {
  ...
  const isPaused =
    runningSinceSeconds === null && elapsedSeconds > 0 && elapsedSeconds < totalSeconds;

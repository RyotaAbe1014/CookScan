...
describe("RecipeEditForm", () => {
  ...
  describe("材料の操作", () => {
    ...
    it("材料を削除できる（2個以上ある場合）", async () => {
      ...
      render(
        <RecipeEditForm tagCategories={mockTagCategories} recipe={recipeWithMultipleIngredients} />,
      );
      ...
      const ingredientRows = screen
        .getAllByPlaceholderText("材料名")
        .map((input) => input.closest(".group"));

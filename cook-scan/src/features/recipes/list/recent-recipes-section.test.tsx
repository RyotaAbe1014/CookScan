...
describe('RecentRecipesSection', () => {
  ...
  test('正常系：セクションタイトルとすべて見るリンクが表示される', () => {
    ...
    expect(screen.getByRole('link', { name: /すべて見る/i })).toHaveAttribute(
      'href',
      '/recipes'
    )
  })
  ...
  test('正常系：3件のレシピカードが表示される', () => {
    ...
    expect(screen.getByRole('link', { name: /パスタ/i })).toHaveAttribute(
      'href',
      '/recipes/2'
    )
    expect(screen.getByRole('link', { name: /親子丼/i })).toHaveAttribute(
      'href',
      '/recipes/3'
    )
  })

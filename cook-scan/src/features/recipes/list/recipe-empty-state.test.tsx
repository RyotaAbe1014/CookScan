...
describe("RecipeEmptyState", () => {
  test("正常系：フィルターなしの場合、レシピがない旨のメッセージが表示される", () => {
    ...
    render(<RecipeEmptyState hasFilters={false} hasSearchQuery={false} hasSelectedTags={false} />);

    // Then: レシピがない旨のメッセージが表示される
    expect(screen.getByText("レシピがまだありません")).toBeInTheDocument();
    expect(
      screen.getByText("レシピをスキャンして、マイレシピに追加しましょう"),
    ).toBeInTheDocument();
  });
  ...
  test("正常系：フィルターありの場合、該当なし旨のメッセージが表示される", () => {
    // Given: フィルターありの空状態
    // When: RecipeEmptyStateをレンダリングする
    render(<RecipeEmptyState hasFilters={true} hasSearchQuery={true} hasSelectedTags={false} />);
  ...
  test("正常系：タグのみの場合、選択タグに一致しないメッセージが表示される", () => {
    // Given: タグのみのフィルター
    // When: RecipeEmptyStateをレンダリングする
    render(<RecipeEmptyState hasFilters={true} hasSearchQuery={false} hasSelectedTags={true} />);

    // Then: 選択タグに一致しないメッセージが表示される
    expect(
      screen.getByText("選択したタグに一致するレシピが見つかりませんでした"),
    ).toBeInTheDocument();
  });
  ...
  test("正常系：検索クエリとタグ両方の場合、両方に言及したメッセージが表示される", () => {
    ...
    // When: RecipeEmptyStateをレンダリングする
    render(<RecipeEmptyState hasFilters={true} hasSearchQuery={true} hasSelectedTags={true} />);

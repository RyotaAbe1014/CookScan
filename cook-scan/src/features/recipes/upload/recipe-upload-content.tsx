  ...
  return (
    <>
      {currentStep !== "method-selection" && (
        <Button variant="secondary" onClick={handleBack} disabled={isUploading} className="mb-6">
          <ChevronLeftIcon className="h-4 w-4" />
          戻る
        </Button>
      )}

      {currentStep === "method-selection" && <MethodSelector onSelect={handleMethodSelect} />}

      {currentStep === "image-upload" && (
        <ImageUpload onUpload={handleImageUpload} onUploadingChange={setIsUploading} />
      )}

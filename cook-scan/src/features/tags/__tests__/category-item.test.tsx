...
describe("CategoryItem", () => {
  ...
  it("カテゴリ更新: 有効な入力で更新するとupdateTagCategoryが呼ばれる", async () => {
    ...
    await waitFor(() => {
      expect(updateTagCategory).toHaveBeenCalledWith(
        "cat-1",
        "更新されたカテゴリ",
        "更新された説明",
      );
  ...
  it("カテゴリ更新: updateTagCategoryがエラーを返した場合、エラーメッセージが表示される", async () => {
    ...
    vi.mocked(updateTagCategory).mockResolvedValueOnce({
      ok: false,
      error: { code: "CONFLICT", message: "同名のカテゴリが存在します" },
    });
  ...
  it("カテゴリ削除: 削除ボタンをクリックすると確認ダイアログが表示される", async () => {
    ...
    expect(mockConfirm).toHaveBeenCalledWith(
      "カテゴリ「料理ジャンル」を削除しますか？この操作は取り消せません。",
    );
  });
  ...
  it("カテゴリ削除: deleteTagCategoryがエラーを返した場合、エラーメッセージが表示される", async () => {
    ...
    vi.mocked(deleteTagCategory).mockResolvedValueOnce({
      ok: false,
      error: { code: "SERVER_ERROR", message: "削除に失敗しました" },
    });
    mockConfirm.mockReturnValue(true);

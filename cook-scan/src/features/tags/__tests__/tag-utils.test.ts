...
describe("validateTagIdsForUser", () => {
  ...
  it("重複フィルタリング: 重複するtagIdは1つにまとめられる", async () => {
    ...
    vi.mocked(prisma.tag.findMany).mockResolvedValue([{ id: "tag-1" }, { id: "tag-2" }] as any);
    ...
    expect(prisma.tag.findMany).toHaveBeenCalledWith({
      where: {
        id: { in: ["tag-1", "tag-2"] },
        OR: [{ isSystem: true }, { user: { id: mockUserId } }],
      },
      select: { id: true },
    });
    expect(result).toEqual({
  ...
  it("無効tagId検出: 他のユーザーのタグは無効", async () => {
    ...
    vi.mocked(prisma.tag.findMany).mockResolvedValue([{ id: "my-tag" }] as any);
